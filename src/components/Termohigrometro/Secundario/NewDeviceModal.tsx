...
export const NewDeviceCard = ({ close }: any) => {
  const dispatch = useAppDispatch();
  const [userid, setUserId] = useState("");
  const [gatewayid, setGatewayId] = useState("");
  const [area, setArea] = useState("");
  const [zona, setZona] = useState("");
  const [zonaAvailable, setZonaAvailable] = useState(true);
  ...
  const handleChangeSelectArea = (event: SelectChangeEvent) => {
    ...
    dispatch(setIdArea(event.target.value))
    setZonaAvailable(false);
    getZonas(Number(event.target.value)).then((data) => {
  ...
  const handleChangeSelectZona = (event: SelectChangeEvent) => {
    ...
    dispatch(setIdZona(event.target.value))
    // console.log(event.target.value);
  };
  ...
  useEffect(() => {
    ...
    if (areasByClient.length === 0) {
      getAreas(Number(idcliente)).then(async (data) => {
        if (data !== undefined) {
          dispatch(setAreasByClient(data));
          const zonaInitial: any = await getZonas(data[0].idarea);
          dispatch(setZonasByAreas(zonaInitial));
        }
      });
    }
  }, []);
